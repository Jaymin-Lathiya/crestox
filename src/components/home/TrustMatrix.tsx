...
import { TRUST_MATRIX_FEATURE_CONTENT, TRUST_MATRIX_SECTION } from '@/config/landingEducationalContent';

const TRUST_ICONS: Record<(typeof TRUST_MATRIX_FEATURE_CONTENT)[number]['icon'], ReactNode> = {
  compliance: (
...
const features = TRUST_MATRIX_FEATURE_CONTENT.map((f) => ({
  ...f,
  icon: TRUST_ICONS[f.icon],
}));
...
const TrustMatrix = () => {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          <p className="terminal-text text-primary text-xs tracking-[0.4em] mb-4">{TRUST_MATRIX_SECTION.kicker}</p>
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl italic text-foreground mb-6">
            {TRUST_MATRIX_SECTION.titleLine1}
            <span className="text-gradient-gold">{TRUST_MATRIX_SECTION.titleAccent}</span>
          </h2>
          <p className="font-sans text-sm text-muted-foreground max-w-xl mx-auto">{TRUST_MATRIX_SECTION.subtitle}</p>
        </motion.div>
  ...
  {features.map((feature, index) => (
    <FeatureFlipCard key={feature.title} feature={feature} index={index} />
        ...
        >
          {['Encrypted Custody', 'Audit-Ready Records', 'Transparent History', 'Compliance-First'].map((badge) => (
